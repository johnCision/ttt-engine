...
function initPort(port) {
    ...
    port.onmessage = message => {
        ...
        if (type === CLIENT_MSG_TYPES.LIST) {
            return serviceHandleListGames(replyPort, data);
        }
        if (type === CLIENT_MSG_TYPES.GAME) {
            return serviceHandleGame(replyPort, data);
        }
        if (type === CLIENT_MSG_TYPES.OFFER) {
            return serviceHandleOfferGame(replyPort, data);
        }
        if (type === CLIENT_MSG_TYPES.CLOSE) {
            return serviceHandleCloseGame(replyPort, data);
        }
        if (type === CLIENT_MSG_TYPES.ACCEPT) {
            return serviceHandleAcceptOffer(replyPort, data);
        }
        if (type === CLIENT_MSG_TYPES.DECLINE) {
            return serviceHandleDeclineOffer(replyPort, data);
        }
        if (type === CLIENT_MSG_TYPES.MOVE) {
            return serviceHandleMove(replyPort, data);
        }
        if (type === CLIENT_MSG_TYPES.FORFEIT) {
            return serviceHandleForfeitGame(replyPort, data);
        }
